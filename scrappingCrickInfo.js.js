...
let responsbe = axios.get(args.source); /* get data from website using axios*/
responsbe.then(function (response) {
    let html = response.data;
       //  console.log(html);


       // part 2
    /* we already have html data but we dont need all this data.
    To look for specific data we look into the browser window from 
    there we get what we need */
    ...
    for (let i = 0; i < matchdivs.length; i++) { // we take out single matches one by one from matchdivs 
        // every matchdivs contain matches information in it
        let matchdiv = matchdivs[i];
        let match = {   // creating match object to get the data in this format
            t1: "",
            t2: "",
            t1s: "",
            t2s: "",
        ...
        let teamParas = matchdiv.querySelectorAll("div.name-detail > p.name");
                // team name and all
        //using the statement given below we can extract the team name 
        match.t1 = teamParas[0].textContent; //   team1 selfteam
        match.t2 = teamParas[1].textContent; // team2 oppteam
    ...
    // making json file matches.json
    let matchesJSON = JSON.stringify(matches); 
    fs.writeFileSync("matches.json", matchesJSON, "utf-8"); 

    // writing function name to manipulate the data 
 ...
 // creating folder for teams with team name
...
// function for excel workbook in which we pass the enteries 
...
// function to write the data in the we need 
